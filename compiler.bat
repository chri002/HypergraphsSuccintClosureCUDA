...
nvcc -rdc=true -D FILE_OUT -D DEBUG  -D CPU -D NTHR=1 -D MAX_BLOCKS_B=16 -D TIME -std=c++17 -lineinfo -Xcompiler -openmp -Xlinker /HEAP:0x8096 .\progetto_G.cu -D HIDE -o .\eseguibile\progetto_16_CPU.exe
nvcc -rdc=true -D FILE_OUT -D DEBUG  -D DYNAMIC -D NTHR=8 -D MAX_BLOCKS_B=16 -D TIME -std=c++17 -lineinfo -Xcompiler -openmp -Xlinker /HEAP:0x8096 .\progetto_G.cu -D HIDE -o .\eseguibile\progetto_16_DYNO.exe
for /l %%a in (2, 1, 6) do ( 
for /l %%b in (1, 1, 5) do ( 
